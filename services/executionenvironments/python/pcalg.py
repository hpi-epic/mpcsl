...
parser.add_argument('--sampling_method', type=str, default="random", help='Data sampling method either random or otherwise top')


def run_pcalg(api_host, job_id, dataset_id, indep_test, alpha, processes, max_level, args, sampling_method, sampling_factor, send_sepsets=False):
    df, ds_load_time = get_dataset(api_host, dataset_id, job_id, sampling_method, sampling_factor)
...
if __name__ == '__main__':
    ...
    run_pcalg(args.api_host, args.job_id, args.dataset_id, indep_test, alpha=args.alpha,
              processes=args.processes, max_level=max_level, args=vars(args), sampling_method=args.sampling_method,
              sampling_factor=args.sampling_factor, send_sepsets=send_sepsets)
