...
def upgrade():
    ...
    op.execute('INSERT INTO experiment_job(job_id, experiment_id, parallel, enforce_cpus, gpus) '\
...
def downgrade():
    op.add_column('job', sa.Column('experiment_id', sa.Integer(), nullable=False))
    ...
    op.execute('UPDATE job SET '\
            ...
            'FROM (SELECT job_id, experiment_id, parallel, enforce_cpus, gpus FROM experiment_job) AS experiment_job '\
            'WHERE job.id = experiment_job.job_id')
