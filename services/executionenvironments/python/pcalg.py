...
parser.add_argument(['--api_host'], type=str, help='API Host/Port', required=True)
parser.add_argument(['-d', '--dataset_id'], type=str, help='Dataset ID', required=True)
...
def run_pcalg(job_id, api_host, dataset_id, indep_test, alpha, processes, max_level, args, send_sepsets=False):
    ...
    start = time.time()
    graph, sepsets = parallel_stable_pc(df, indep_test, alpha=alpha, processes=processes,
